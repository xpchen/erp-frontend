...
// 定义一个对象类型，用于传递参数，有两个字段，一个id,一个itemId
export interface RequestItem {
  sourceBillId: number, //采购申请单id
...
export const PurchaseOrderApi = {
  ...
  ,
  getPurchaseRequestItemsListByRequestIds: async (requestItems: RequestItem[]) => {
    // POST方式获取采购申请单明细列表，参数为requestIds数组
    return await request.post({ url: `/erp/purchase-order/create-order-from-request`, data: requestItems })
