...
const categoryList = ref<MaterialCategoryVO[]>([]) // 产品分类列表
...
const handleUpdate = (id: number) => {
  push('/erp/product/edit?id=' + id + '&type=update')
}
...
watch(() => route.fullPath, async () => {
      ...
      // 刷新列表
      if(route.fullPath == "/erp/product/product"){
        await getList()
      }
    });
...
onMounted(async () => {
  await getList()
  // 产品分类
  const categoryData = await MaterialCategoryApi.getMaterialCategorySimpleList();
