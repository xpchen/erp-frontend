<template>
  <Dialog :title="dialogTitle" v-model="dialogVisible" width="1440">
    ...
    >
      ...
      <el-row :gutter="20">
        <el-col :span="8" v-if="checkPermi(['erp:amount:view'])">
...
const handlePurchaseOrderChange = (order: PurchaseOrderVO) => {
  ...
  order.items.forEach((item) => {
    ...
    item.count = item.totalCount - item.inCount + item.returnCount
...
const resetForm = () => {
  ...
  formData.value = {
    ...
    accountId: defaultAccountId.value,
    // 新增时将入库时间设为当前时间
    inTime: formType.value === 'create' ? currentTime : undefined, 
    remark: undefined,
    fileUrl: undefined,
    discountPercent: 0,
    discountPrice: 0,
