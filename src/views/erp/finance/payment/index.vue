  ...
  <ContentWrap>
    ...
    >
      ...
      <el-form-item>
        ...
        <el-button type="default" plain @click="openStatementDialog(1)" v-hasPermi="['erp:finance-payment:select-statement']">
          <Icon icon="ep:document" class="mr-5px" /> 生成付款单
        </el-button>
      </el-form-item>
    </el-form>
  </ContentWrap>
...
const handleStatementSelect = async (data: { statementNos: string[], settleTheBill: boolean }) => {
  debugger
   try {
    // 调用API生成支付信息
    await FinancePaymentApi.generatePayment(data);
    // 成功后刷新列表
    await getList();
  } catch (error) {
