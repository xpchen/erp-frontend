<template>
    ...
    >
      ...
      <el-form-item>
        ...
        <el-button
          type="danger"
          plain
          @click="handleDelete(selectionList.filter(item => item.status === 10).map((item) => item.id))"
        ...
        <el-button type="default" plain @click="openStatementDialog(2)" v-hasPermi="['erp:finance-receipt:select-statement']">
          <Icon icon="ep:document" class="mr-5px" /> 生成收款单
        </el-button>
      </el-form-item>
    </el-form>
  </ContentWrap>
  ...
  <FinanceStatementSelectDialog ref="statementDialogRef" @success="handleStatementSelect" />
</template>
...
import FinanceStatementSelectDialog from '../financestatement/components/FinanceStatementSelectDialog.vue'
...
const handleDelete = async (ids: number[]) => {
  try {
    if(ids.length === 0){
      message.warning('已审批的数据不能被删除，请先进行反审批操作')
      return
    }
    // 删除的二次确认
...
const statementDialogRef = ref() // 对账单选择对话框引用

/** 打开对账单选择对话框 */
const openStatementDialog = (financeStatementType: number) => {
  statementDialogRef.value?.open(financeStatementType)
}

const handleStatementSelect = async (data: { statementNos: string[], settleTheBill: boolean }) => {
  debugger
   try {
    // 调用API生成收款单
    await FinanceReceiptApi.generateReceipt(data);
    // 成功后刷新列表
    await getList();
  } catch (error) {
    console.error('生成收款单失败:', error);
  }
}
</script>
